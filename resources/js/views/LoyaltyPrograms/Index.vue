<template>
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-3xl font-bold text-gray-900">LoyaltyPrograms</h1>
        <p class="mt-2 text-sm text-gray-600">Manage your loyaltyprograms</p>
      </div>
      <button
        @click="openCreateModal"
        class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-violet-600 hover:bg-violet-700"
      >
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="item in items" :key="item.id">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
              <button
                @click="editItem(item)"
                class="text-violet-600 hover:text-violet-900 mr-4"
              >
                Edit
              </button>
              <button
    ...
    <FormModal
      v-model="showModal"
      :title="editingItem ? 'Edit LoyaltyProgram' : 'Create LoyaltyProgram'"
      @save="saveItem"
    >
