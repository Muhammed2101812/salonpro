<template>
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-3xl font-bold text-gray-900">MarketingCampaigns</h1>
        <p class="mt-2 text-sm text-gray-600">Manage your marketingcampaigns</p>
      </div>
      <button
        @click="openCreateModal"
        class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-rose-600 hover:bg-rose-700"
      >
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="item in items" :key="item.id">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
              <button
                @click="editItem(item)"
                class="text-rose-600 hover:text-rose-900 mr-4"
              >
                Edit
              </button>
              <button
    ...
    <FormModal
      v-model="showModal"
      :title="editingItem ? 'Edit MarketingCampaign' : 'Create MarketingCampaign'"
      @save="saveItem"
    >
