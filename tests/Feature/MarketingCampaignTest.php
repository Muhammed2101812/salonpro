<?php

declare(strict_types=1);

namespace Tests\Feature;

use App\Models\MarketingCampaign;
use App\Models\User;
use Illuminate\Foundation\Testing\RefreshDatabase;
use Tests\TestCase;

class MarketingCampaignTest extends TestCase
{
    use RefreshDatabase;

    protected User $user;

    protected function setUp(): void
    {
        parent::setUp();
        $this->user = User::factory()->create();
    }

    public function test_can_list_marketingCampaigns(): void
    {
        MarketingCampaign::factory()->count(3)->create();

        $response = $this->actingAs($this->user, 'sanctum')
            ->getJson('/api/v1/marketing-campaigns');

        $response->assertOk()
            ->assertJsonStructure([
                'data' => [
                    '*' => ['id']
                ]
            ]);
    }

    public function test_can_create_marketingCampaign(): void
    {
        $data = MarketingCampaign::factory()->make()->toArray();

        $response = $this->actingAs($this->user, 'sanctum')
            ->postJson('/api/v1/marketing-campaigns', $data);

        $response->assertCreated()
            ->assertJsonStructure([
                'success',
                'message',
                'data' => ['id']
            ]);

        $this->assertDatabaseHas('marketing-campaigns', [
            'id' => $response->json('data.id')
        ]);
    }

    public function test_can_show_marketingCampaign(): void
    {
        $marketingCampaign = MarketingCampaign::factory()->create();

        $response = $this->actingAs($this->user, 'sanctum')
            ->getJson("/api/v1/marketing-campaigns/{$marketingCampaign->id}");

        $response->assertOk()
            ->assertJsonStructure([
                'success',
                'data' => ['id']
            ]);
    }

    public function test_can_update_marketingCampaign(): void
    {
        $marketingCampaign = MarketingCampaign::factory()->create();
        $updateData = MarketingCampaign::factory()->make()->toArray();

        $response = $this->actingAs($this->user, 'sanctum')
            ->putJson("/api/v1/marketing-campaigns/{$marketingCampaign->id}", $updateData);

        $response->assertOk()
            ->assertJsonStructure([
                'success',
                'message',
                'data' => ['id']
            ]);
    }

    public function test_can_delete_marketingCampaign(): void
    {
        $marketingCampaign = MarketingCampaign::factory()->create();

        $response = $this->actingAs($this->user, 'sanctum')
            ->deleteJson("/api/v1/marketing-campaigns/{$marketingCampaign->id}");

        $response->assertOk()
            ->assertJson([
                'success' => true
            ]);

        $this->assertSoftDeleted('marketing-campaigns', [
            'id' => $marketingCampaign->id
        ]);
    }

    public function test_unauthorized_access_is_denied(): void
    {
        $response = $this->getJson('/api/v1/marketing-campaigns');

        $response->assertUnauthorized();
    }
}
