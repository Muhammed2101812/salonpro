...
class ActivityLogTest extends TestCase
{
    ...
    public function test_can_list_activityLogs(): void
    {
    ...
    public function test_can_create_activityLog(): void
    {
        ...
        $response = $this->actingAs($this->user, 'sanctum')
            ->postJson('/api/v1/activity-logs', $data);
        ...
        $this->assertDatabaseHas('activity-logs', [
            'id' => $response->json('data.id')
        ]);
    }

    public function test_can_show_activityLog(): void
    {
    ...
    public function test_can_update_activityLog(): void
    {
        ...
        $response = $this->actingAs($this->user, 'sanctum')
            ->putJson("/api/v1/activity-logs/{$activityLog->id}", $updateData);
    ...
    public function test_can_delete_activityLog(): void
    {
        ...
        $response = $this->actingAs($this->user, 'sanctum')
            ->deleteJson("/api/v1/activity-logs/{$activityLog->id}");
        ...
        $this->assertSoftDeleted('activity-logs', [
            'id' => $activityLog->id
        ]);
    }
