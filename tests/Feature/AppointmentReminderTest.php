...
class AppointmentReminderTest extends TestCase
{
    ...
    public function test_can_list_appointmentReminders(): void
    {
    ...
    public function test_can_create_appointmentReminder(): void
    {
        ...
        $response = $this->actingAs($this->user, 'sanctum')
            ->postJson('/api/v1/appointment-reminders', $data);
        ...
        $this->assertDatabaseHas('appointment-reminders', [
            'id' => $response->json('data.id')
        ]);
    }

    public function test_can_show_appointmentReminder(): void
    {
        $appointmentReminder = AppointmentReminder::factory()->create();

        $response = $this->actingAs($this->user, 'sanctum')
            ->getJson("/api/v1/appointment-reminders/{$appointmentReminder->id}");
    ...
    public function test_can_update_appointmentReminder(): void
    {
        ...
        $response = $this->actingAs($this->user, 'sanctum')
            ->putJson("/api/v1/appointment-reminders/{$appointmentReminder->id}", $updateData);
    ...
    public function test_can_delete_appointmentReminder(): void
    {
        ...
        $response = $this->actingAs($this->user, 'sanctum')
            ->deleteJson("/api/v1/appointment-reminders/{$appointmentReminder->id}");
        ...
        $this->assertSoftDeleted('appointment-reminders', [
