...
class StockTransferTest extends TestCase
{
    ...
    public function test_can_list_stockTransfers(): void
    {
    ...
    public function test_can_create_stockTransfer(): void
    {
        ...
        $response = $this->actingAs($this->user, 'sanctum')
            ->postJson('/api/v1/stock-transfers', $data);
        ...
        $this->assertDatabaseHas('stock-transfers', [
            'id' => $response->json('data.id')
        ]);
    }

    public function test_can_show_stockTransfer(): void
    {
        $stockTransfer = StockTransfer::factory()->create();

        $response = $this->actingAs($this->user, 'sanctum')
            ->getJson("/api/v1/stock-transfers/{$stockTransfer->id}");
    ...
    public function test_can_update_stockTransfer(): void
    {
        ...
        $response = $this->actingAs($this->user, 'sanctum')
            ->putJson("/api/v1/stock-transfers/{$stockTransfer->id}", $updateData);
    ...
    public function test_can_delete_stockTransfer(): void
    {
        ...
        $response = $this->actingAs($this->user, 'sanctum')
            ->deleteJson("/api/v1/stock-transfers/{$stockTransfer->id}");
        ...
        $this->assertSoftDeleted('stock-transfers', [
            'id' => $stockTransfer->id
        ]);
    }
